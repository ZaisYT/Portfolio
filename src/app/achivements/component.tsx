...
export default function MainComponent(props: MainProps) {
  const { routes } = props;
  ...
  if (!stats) {
    return (
      <div className="min-h-screen p-4 bg-background-800">
        <Header />
        <h1 className="font-Lilita_One text-accent-700 text-5xl mb-4">
          Logros :3
        </h1>
  ...
  function percentageCalculation(val: number, expected: number): number {
    return Math.min((val / expected) * 100, 100);
  }
  ...
  function percentageAllAchivements(): number {
    ...
    let achivementsCompleted = 0;

    if (closedWindows) achivementsCompleted++;
    if (sectionSwitches) achivementsCompleted++;

    if (closedWindows >= 20) achivementsCompleted++;
    if (sectionSwitches >= 20) achivementsCompleted++;

    if (closedWindows >= 100) achivementsCompleted++;
    if (sectionSwitches >= 100) achivementsCompleted++;

    let infoAchivementsCalc = infoClicked
      ? Math.floor(Math.log10(infoClicked)) + 1
      : 0;
    if (infoAchivementsCalc > 3) infoAchivementsCalc = 3;
    achivementsCompleted += infoAchivementsCalc;

    if (isKonami) achivementsCompleted++;
    if (isFOFDiscovered()) achivementsCompleted++;
    if (pagesVisited.length == routes.length) achivementsCompleted++;

    return (achivementsCompleted / TotalAchivements) * 100;
  }
  ...
  const achievements = [
    ...
    {
      val: percentageCalculation(stats.pagesVisited.length, routes.length),
      Title: "100%?",
      ...
      <div>
        {achievements.map((ach, index) => (
          <Achievement
            key={index}
            curVal={ach.val}
            data={{ ...ach, Unlocked: isCompleted(ach.val) }}
          />
        ))}
      </div>
    </div>
...
const Achievement = (props: AchiType) => {
  const { curVal, data } = props;
  return (
      ...
      <div className="flex">
        <Image
          ...
          className={`w-24 h-24 rounded-md mr-2 ${
            data.Unlocked ? "" : "grayscale opacity-40"
          }`}
        />
        <div>
          <h1 className="text-white font-Lilita_One text-xl">{data.Title}</h1>
          <h2 className="text-neutral-400 font-Afacad_Flux text-md">
            {data.Desc}
          </h2>
        </div>
      </div>
      ...
      <div className="flex flex-col mt-2">
        <div className="h-2 w-full bg-neutral-700 rounded-full">
          <div
            className={`h-2.5 rounded-full ${
              data.Unlocked ? "bg-primary-500" : "bg-secondary-500"
            }`}
            style={{ width: `${curVal}%` }}
          ></div>
        </div>
        <div className="flex justify-between">
          <h2 className="text-neutral-400 font-Afacad_Flux">
            {String(
              Number.isInteger(Number(curVal)) ? curVal : curVal.toFixed(2)
            )}
            %
          </h2>
